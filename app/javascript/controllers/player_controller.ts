...
export default class PlayerController extends Controller<HTMLElement> {
    static targets = [
        ...
        "cassetteContainer", "cassetteShowTitle", "cassetteShowLocation", "cassetteDate", "cassetteDuration",
        "cassetteSide"
    ];
    ...
    declare readonly cassetteContainerTarget: HTMLElement;
    declare readonly cassetteShowTitleTarget: HTMLElement;
    declare readonly cassetteShowLocationTarget: HTMLElement;
    declare readonly cassetteDateTarget: HTMLElement;
    declare readonly cassetteDurationTarget: HTMLElement;
    declare readonly cassetteSideTarget: HTMLElement;
    ...
    private setCassetteInfo(playlist: Playlist | null) {
        this.cassetteContainerTarget.style.setProperty(
            '--cassette-label-color',
            playlist?.color?.length > 0 ? `rgb(${playlist.color})` : '#ff4400'
        );

        const hasTitle = playlist?.showTitle?.length > 0;

        this.cassetteShowTitleTarget.innerText = hasTitle ? playlist.showTitle : playlist?.venueName;
        this.cassetteShowLocationTarget.innerHTML =
            hasTitle ? `${playlist.venueName} &bull; ${playlist.city}` : playlist?.city;

        this.cassetteDateTarget.innerText = playlist?.date;

        this.cassetteDurationTarget.innerText = playlist ? (Math.ceil(playlist.duration / 900) * 15).toString() : '90';
    }

    private setCassetteSide(track: Track | null) {
        if (track == null) {
            this.cassetteSideTarget.innerText = 'A';
            return;
        }

        const trackIdx = this.playlist.tracks.findIndex(t => t.url === track.url);

        if (trackIdx < Math.ceil(this.playlist.tracks.length / 2)) {
            this.cassetteSideTarget.innerText = 'A';
        }
        else {
            this.cassetteSideTarget.innerText = 'B';
        }
    }
}
